...
- **M3 (done) — Scheduler + real multi-tenant isolation + git archive + global rate limiter/per-profile run cap.** `scheduler/run_due_profiles.py` — a one-shot sweep meant to be invoked by the host's own cron (Render Cron Job / OS cron), not a long-running daemon. Cron due-ness + the hard 1-run/24h cap live in `packages/core/schedule.py` (shared with the M2 manual "Run search now" endpoint, so neither path bypasses the cap); the global per-source_type token bucket lives in `packages/core/rate_limiter.py`, enforced centrally in `pipeline.py`'s fetch step. Git archive commits land via `packages/outputs/git_archive.py` (no-op if `JOBFIGHTER_DATA_REPO_PATH` is unset). Demo: 24h+ unattended run across several seeded profiles on different schedules; confirm per-user isolation on an induced failure; confirm real git-archive commits with sensible diffs.
